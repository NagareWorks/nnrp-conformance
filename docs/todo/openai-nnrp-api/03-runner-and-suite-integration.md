...
- [x] Make API profile selection depend on both the selected protocol baseline and the API capability manifest.
...
- [x] Preserve not-claimed cases as compatibility matrix entries rather than failures.
...
- [x] Validate error bodies and cancellation terminal outcomes.
- [x] Validate that non-critical extension fields do not affect baseline pass/fail.
- [x] Emit compatibility matrix output for API profile coverage.
