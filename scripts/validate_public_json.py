...
def validate_protocol_composition(
    ...
    for relative_path in protocol_manifest.get("case_manifests", []):
        ...
        cases = case_manifest.get("cases")
        if not isinstance(cases, list):
            raise SystemExit(f"case manifest cases must be an array: {case_path}")
        for case in cases:
            if not isinstance(case, dict):
                raise SystemExit(f"case manifest contains a non-object case: {case_path}")
            if not isinstance(case.get("id"), str) or not case["id"]:
                raise SystemExit(f"case manifest contains an invalid case id: {case_path}")
            case_id = case["id"]
    ...
    for relative_path in protocol_manifest.get("vector_recipe_manifests", []):
        ...
        vectors = vector_manifest.get("vectors")
        if not isinstance(vectors, list):
            raise SystemExit(f"vector recipe manifest vectors must be an array: {vector_path}")
        for vector in vectors:
            if not isinstance(vector, dict):
                raise SystemExit(
                    f"vector recipe manifest contains a non-object vector: {vector_path}"
                )
            if not isinstance(vector.get("name"), str) or not vector["name"]:
                raise SystemExit(
                    f"vector recipe manifest contains an invalid vector name: {vector_path}"
                )
            name = vector["name"]
